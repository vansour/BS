...
# Detector-only recovery run.
#
# Goal:
# - Start from the current formal fog-focused checkpoint
# - Re-enable detector optimization
# - Disable fog classification / beta regression updates
# - Use only clear-image detection loss to recover clear-weather detection
#
# Usage:
#   bash scripts/run_detector_recovery_train.sh

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

DEFAULT_OUTPUT_DIR="$ROOT_DIR/outputs/Fog_Detection_Project_detectorrecovery"
...
export BS_CLEAR_DET_LOSS_WEIGHT="${BS_CLEAR_DET_LOSS_WEIGHT:-1.0}"
export BS_FOG_CLS_LOSS_WEIGHT="${BS_FOG_CLS_LOSS_WEIGHT:-0.0}"
export BS_FOG_REG_LOSS_WEIGHT="${BS_FOG_REG_LOSS_WEIGHT:-0.0}"
...
RUN_LOG="$BS_OUTPUT_DIR/detector_recovery_train_${RUN_STAMP}.log"

echo "Detector-only recovery run"
