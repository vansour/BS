...
# COCO-vehicle-head pilot run.
#
# Goal:
# - Keep the original COCO 80-class detector head instead of rebuilding a single-class head
# - Reuse fog heads and compatible backbone weights from the formal fogfocus_full checkpoint
# - Map UA-DETRAC training targets onto the COCO `car` class during training
# - Collapse COCO vehicle classes back to `vehicle` at inference time
#
# Usage:
...
DEFAULT_OUTPUT_DIR="$ROOT_DIR/outputs/Fog_Detection_Project_cocohead_pilot"
DEFAULT_CHECKPOINT_DIR="$DEFAULT_OUTPUT_DIR/checkpoints"
DEFAULT_RESUME_CHECKPOINT="$ROOT_DIR/outputs/Fog_Detection_Project_fogfocus_full/unified_model_best.pt"

export BS_CONFIG_FILE="${BS_CONFIG_FILE:-$ROOT_DIR/configs/coco_vehicle_head_pilot.yaml}"
...
export BS_DET_LOSS_WEIGHT="${BS_DET_LOSS_WEIGHT:-0.75}"
export BS_CLEAR_DET_LOSS_WEIGHT="${BS_CLEAR_DET_LOSS_WEIGHT:-0.35}"
export BS_FOG_CLS_LOSS_WEIGHT="${BS_FOG_CLS_LOSS_WEIGHT:-1.25}"
export BS_FOG_REG_LOSS_WEIGHT="${BS_FOG_REG_LOSS_WEIGHT:-1.0}"
...
RUN_LOG="$BS_OUTPUT_DIR/coco_vehicle_head_pilot_${RUN_STAMP}.log"

echo "COCO vehicle-head pilot run"
echo "ROOT_DIR=$ROOT_DIR"
echo "BS_CONFIG_FILE=$BS_CONFIG_FILE"
...
echo "BS_FRAME_STRIDE=$BS_FRAME_STRIDE"
echo "BS_LR=$BS_LR"
echo "BS_DET_LOSS_WEIGHT=$BS_DET_LOSS_WEIGHT"
echo "BS_CLEAR_DET_LOSS_WEIGHT=$BS_CLEAR_DET_LOSS_WEIGHT"
echo "BS_FOG_CLS_LOSS_WEIGHT=$BS_FOG_CLS_LOSS_WEIGHT"
echo "BS_FOG_REG_LOSS_WEIGHT=$BS_FOG_REG_LOSS_WEIGHT"
